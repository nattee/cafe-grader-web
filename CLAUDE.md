...
- **Table Action Columns:** Standardize row-level table actions (edit, delete, stat) into a single, right-aligned compact column (`<td class="align-middle py-1 pr-2">`). Group the action buttons inside a `<div class="d-flex gap-1 justify-content-end">` container and use borderless outline pill buttons (`class="btn btn-flex btn-outline-{color} border-0"` - do not use `btn-sm`). Ensure that Material Icons (`.mi`) within these buttons use the default 24px size (do not apply `.md-18` or other custom sizes) to maintain symmetry and legibility. The tightened row padding (`py-1`) maximizes data density while maintaining a unified, modern aesthetic with comfortably sized click targets.
