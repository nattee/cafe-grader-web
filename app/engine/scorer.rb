...
class Scorer
  ...
  def score(sub,testcase)
    ...
    if (status.exitstatus == 0)
      judge_log "scoring of sub ##{@sub.id} testcase ##{@testcase.id}: 1 (correct)"
      return report_score_correct
    else
      judge_log "scoring of sub ##{@sub.id} testcase ##{@testcase.id}: 0 (wrong answer)"
      return report_score_wrong
    end

  end

  def report_score_correct(score = 1.to_d)
    {
      result: :correct,
      score: score
    }
  ...
  def report_score_wrong(score = 0.to_d)
    {
      result: :wrong,
      score: score
    }
  ...
  def report_score_partial(score)
    {
      result: :partial,
      score: score
    }
