...
- **Change:** SHIPPED on master (revs 2134–2143): dedicated `viva` queue+worker so assist traffic can't starve interviews (2137); per-call timing columns `llm_started_at`/`llm_latency_ms` splitting queue wait from provider time (2135–2136); the stale-turn sweeper now distinguishes queued from running so a backlog no longer shows a false timeout (2142); the contest AI-usage report (2140–2141); header 500 fix (2139); admin-only site-mode switch (2141). Deployment still needed (queue worker RAILS_MAX_THREADS>=12); not yet merged to chula_cp or deployed.
