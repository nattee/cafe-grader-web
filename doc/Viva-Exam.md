...
- **Apply** queues one job per target with a `batch_id` (`regrade-<problem id>-<timestamp>`) and writes one `viva_regrade` audit row on the problem (batch id, contest, model, rule, rubric version, target ids). Watch the queue on `/grader_processes/queues`. `viva:regrade_status` reads that audit row and prints adopted / lower / error / reverted / undecided / pending counts, the old / new / final means and up / equal / down, then one line per target; `CSV=` writes the rows. `viva:regrade_revert` re-adopts each replaced run (displaced runs become `reverted`, nothing is deleted) and writes a `viva_regrade_revert` audit row; a target that had no valid grade before the batch keeps its new grade.
